...
class WPEX_Testimonials_Config {
	...
	public function __construct() {
		...
		/*-------------------------------------------------------------------------------*/
			...
			if ( wpex_get_mod( 'testimonials_custom_sidebar', true ) ) {
				add_filter( 'wpex_get_sidebar', array( 'WPEX_Testimonials_Config', 'display_sidebar' ), 10, 2 );
			}

			// Alter the default page title
			add_action( 'wpex_page_header_title_args', array( 'WPEX_Testimonials_Config', 'alter_title' ), 10, 2 );

			// Alter the post layouts for testimonials posts and archives
			add_filter( 'wpex_post_layout_class', array( 'WPEX_Testimonials_Config', 'layouts' ), 10, 2 );
	...
	public static function display_sidebar( $sidebar, $instance ) {
		if ( 'singular_testimonials' == $instance || ( 'archive' == $instance && wpex_is_testimonials_tax() ) ) {
	...
	public static function alter_title( $args, $instance ) {
		if ( 'singular_testimonials' == $instance ) {
	...
	public static function layouts( $class, $instance ) {
		...
		} elseif ( 'tax' == $instance && wpex_is_testimonials_tax() ) {
