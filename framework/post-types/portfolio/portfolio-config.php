...
class WPEX_Portfolio_Config {
	...
	public function __construct() {
		...
		/*-------------------------------------------------------------------------------*/
			...
			if ( wpex_get_mod( 'portfolio_custom_sidebar', true ) ) {
				add_filter( 'wpex_get_sidebar', array( 'WPEX_Portfolio_Config', 'display_sidebar' ), 10, 2 );
			}

			// Alter the post layouts for portfolio posts and archives
			add_filter( 'wpex_post_layout_class', array( 'WPEX_Portfolio_Config', 'layouts' ), 10, 2 );
			...
			add_filter( 'wpex_page_header_title_args', array( 'WPEX_Portfolio_Config', 'alter_title' ), 10, 2 );
	...
	public static function display_sidebar( $sidebar, $instance ) {
		if ( 'singular_portfolio' == $instance || ( 'archive' == $instance && wpex_is_portfolio_tax() ) ) {
	...
	public static function layouts( $class, $instance ) {
		...
		} elseif ( 'tax' == $instance && wpex_is_portfolio_tax() ) {
