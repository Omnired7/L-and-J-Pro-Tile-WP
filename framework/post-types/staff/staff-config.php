...
class WPEX_Staff_Config {
	...
	public function __construct() {
		...
		/*-------------------------------------------------------------------------------*/
			...
			if ( wpex_get_mod( 'staff_custom_sidebar', true ) ) {
				add_filter( 'wpex_get_sidebar', array( 'WPEX_Staff_Config', 'display_sidebar' ), 10, 2 );
			}

			// Alter the post layouts for staff posts and archives
			add_filter( 'wpex_post_layout_class', array( 'WPEX_Staff_Config', 'layouts' ), 10, 2 );

			// Add subheading for staff member if enabled
			add_action( 'wpex_post_subheading', array( 'WPEX_Staff_Config', 'add_position_to_subheading' ), 10, 2 );
			...
			add_filter( 'wpex_page_header_title_args', array( 'WPEX_Staff_Config', 'alter_title' ), 10, 2 );
	 ...
	 * @since 2.0.0
	 */
	public static function display_sidebar( $sidebar, $instance ) {
		if ( 'singular_staff' == $instance || ( 'archive' == $instance && wpex_is_staff_tax() ) ) {
	 ...
	 * @since 2.0.0
	 */
	public static function layouts( $class, $instance ) {
		if ( 'singular_staff' == $instance ) {
			$class = wpex_get_mod( 'staff_single_layout', 'right-sidebar' );
		} elseif ( 'tax' == $instance && wpex_is_staff_tax() ) {
	 ...
	 * @since 2.0.0
	 */
	public static function add_position_to_subheading( $subheading, $instance ) {
